...
/// Run `validate_tag_nesting` and return errors.
fn run(events: &[TagEvent]) -> Vec<HtmlError> {
    let mut errors = Vec::new();
    validate_tag_nesting(events, &mut errors);
    errors
}
...
mod adversarial {
    ...
    mod pathological {
        ...
        #[test]
        fn empty_tag_name() {
            // Shouldn't happen from tokenizer, but validate_tag_nesting shouldn't panic.
