...
/// Alias for Cow<str> - zero-copy string that may be borrowed or owned.
...
impl<'s> Attributes<'s> {
    ...
    pub fn contains_key(&self, key: &str) -> bool {
        self.0.iter().any(|(k, _)| matches!(k.key(), Some(k) if k == key))
    }
    ...
    pub fn get_value(&self, key: &str) -> Option<AttributeValue<'_>> {
        if key == "class" {
            ...
            if value.raw.is_empty() { None } else { Some(value) }
        } else {
            self.0
                .iter()
    ...
    pub fn unique_pairs<'a>(&'a self) -> AttributePairsIter<'a, 's> {
        AttributePairsIter { attrs: &self.0, pos: 0 }
    }
}
...
impl<'a: 's, 's> Iterator for AttributePairsIter<'a, 's> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while let Some((key, value)) = self.attrs[self.pos..].first() {
            ...
            if self.attrs[..self.pos - 1].iter().any(|(k, _)| k.key() == Some(key)) {
            ...
            if let Some((_, v)) = self.attrs[self.pos..].iter().rfind(|(k, _)| k.key() == Some(key)) {
        ...
        for (pos, c) in input.bytes().enumerate() {
            ...
            if st != self.state && !matches!((st, self.state), (ValueEscape, _) | (_, ValueEscape)) {
                ...
                match st {
                    ...
                    Key => self
                        .attrs
                        .push((AttributeKind::Pair { key: content.into() }, "".into())),
                    ...
                    Comment | CommentNewline => {
                        ...
                        self.attrs.0[last]
                            .1
                            .extend_raw(if matches!(st, Comment) { content } else { "\n" });
                    }
